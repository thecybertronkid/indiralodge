...
export async function GET(req: Request) {
  try {
    ...
    // 1. Occupancy & Room Counts
    const totalRooms = await db.room.count({ where: { propertyId, isActive: true } });
    const occupiedRooms = await db.room.count({ where: { propertyId, isActive: true, availabilityStatus: 'OCCUPIED' } });
    const reservedRooms = await db.room.count({ where: { propertyId, isActive: true, availabilityStatus: 'RESERVED' } });
    const outOfOrderRooms = await db.room.count({
      where: {
        propertyId,
        isActive: true,
    ...
    // 4. In-House Guest List
    const inHouseGuests = await db.reservation.findMany({
      where: {
        propertyId,
        status: 'CHECKED_IN',
      },
