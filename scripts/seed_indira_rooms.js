...
async function main() {
  console.log('======================================================');
  console.log('  SEEDING OFFICIAL INDIRA LODGE ROOM TYPES & RATES    ');
  console.log('======================================================');

  let org = await prisma.organization.findFirst();
  if (!org) {
    org = await prisma.organization.create({
      data: { name: 'Indira Lodge Group', code: 'ORG-INDIRA-01' }
    });
  }

  let prop = await prisma.property.findFirst({ where: { organizationId: org.id } });
  if (!prop) {
  ...
  const createdTypesMap = {};

  for (const rt of officialRoomTypes) {
    const existing = await prisma.roomType.findUnique({
      where: { propertyId_code: { propertyId: prop.id, code: rt.code } },
    });
    ...
    if (existing) {
      ...
      console.log(`Updated room type: ${rt.name} (${rt.code})`);
    } else {
      const created = await prisma.roomType.create({
        data: { propertyId: prop.id, ...rt },
      });
      createdTypesMap[rt.code] = created.id;
      console.log(`Created room type: ${rt.name} (${rt.code})`);
    }
  }
  ...
  for (const dr of defaultRooms) {
    ...
    const existingRoom = await prisma.room.findUnique({
      where: { propertyId_roomNumber: { propertyId: prop.id, roomNumber: dr.roomNumber } },
    });
    ...
    if (!existingRoom) {
      ...
      console.log(`Created Room ${dr.roomNumber} (${dr.code})`);
    } else {
      await prisma.room.update({
        where: { id: existingRoom.id },
        data: { roomTypeId: rtId },
      });
      console.log(`Updated Room ${dr.roomNumber} -> (${dr.code})`);
    }
  }

  console.log('======================================================');
  console.log('[SUCCESS] All 9 Indira Lodge room categories & rooms ready!');
  console.log('======================================================');
}
